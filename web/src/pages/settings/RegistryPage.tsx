import { zodResolver } from "@hookform/resolvers/zod";
import { AlertTriangle } from "lucide-react";
import { useEffect, useMemo, useRef } from "react";
import { type Control, type FieldErrors, useForm } from "react-hook-form";
import { Link } from "react-router-dom";
import { toast } from "sonner";
import type { ResolvedSetting, SettingsMap } from "@/api/types";
import { EmptyState } from "@/components/layout/EmptyState";
import { PageHeader } from "@/components/layout/PageHeader";
import { SectionCard } from "@/components/layout/SectionCard";
import { Button } from "@/components/ui/button";
import { Skeleton } from "@/components/ui/skeleton";
import { useQueryErrorToast } from "@/hooks/useQueryErrorToast";
import { useSettings, useUpdateSettings } from "@/hooks/useSettings";
import type { SettingsPageInfo } from "@/lib/settingsPages";
import { AuthSection } from "@/pages/settings/AuthSection";
import { LlmModelsGroup, LlmProviderGroup, RateLimitLatch } from "@/pages/settings/LlmGroups";
import { SettingField } from "@/pages/settings/SettingField";
import {
  buildSettingsSchema,
  groupEntries,
  groupFor,
  type RegistryPageId,
  type SettingsFormValues,
  sectionFor,
  toFormValues,
  toPatch,
} from "@/pages/settings/schema";
import { useOpenGroups } from "@/pages/settings/useOpenGroups";

/** The registry entries one page owns, as a map the schema helpers take. */
function pageSettings(settings: SettingsMap, page: RegistryPageId): SettingsMap {
  return Object.fromEntries(Object.entries(settings).filter(([key]) => sectionFor(key) === page));
}

/**
 * A settings page generated from the registry `GET /api/settings` returns.
 *
 * Adding a setting in `gaggiclanker/settings.py` makes it appear on the page
 * its prefix names, with the right control and the right validation — that is
 * the whole point of the registry. The page is its own form over its own keys:
 * Save sends what changed here and nothing else, and a value on another page
 * that this browser would not accept cannot block it.
 */
export function RegistryPage({ page }: { page: SettingsPageInfo & { id: RegistryPageId } }) {
  const settingsQuery = useSettings();
  const update = useUpdateSettings();
  const { isOpen, setGroupOpen, openGroups } = useOpenGroups();

  useQueryErrorToast(settingsQuery.error, "Could not load settings");

  const all = settingsQuery.data;
  const settings = useMemo(() => (all ? pageSettings(all, page.id) : undefined), [all, page.id]);
  const schema = useMemo(() => (settings ? buildSettingsSchema(settings) : undefined), [settings]);
  const groups = useMemo(
    () => groupEntries(page.id, Object.values(settings ?? {})),
    [page.id, settings],
  );

  const form = useForm<SettingsFormValues>({
    // The resolver depends on data that arrives after the first render, so it
    // is swapped in rather than built once — react-hook-form reads it per
    // submit, which is why this works.
    resolver: schema ? zodResolver(schema) : undefined,
    defaultValues: {},
    mode: "onSubmit",
  });

  const { reset } = form;

  // Hydrate once, and then only while the form is pristine.
  //
  // Every refetch hands back a fresh object, and `onSettled` refetches after a
  // FAILED save too — so a plain `reset` on every change silently threw away
  // what the user had just typed, at precisely the moment they most wanted to
  // keep it and retry. The dirty flag is read through a ref so it does not
  // become an effect dependency and re-trigger the reset it just caused.
  const isDirtyRef = useRef(false);
  isDirtyRef.current = form.formState.isDirty;
  useEffect(() => {
    if (settings && !isDirtyRef.current) reset(toFormValues(settings));
  }, [settings, reset]);

  const onSubmit = form.handleSubmit(
    async (values) => {
      if (!settings) return;
      const patch = toPatch(settings, values);
      if (Object.keys(patch).length === 0) {
        toast.info("Nothing to save");
        return;
      }
      try {
        const next = await update.mutateAsync(patch);
        // Re-seed from the server's answer: it is what decided the new source
        // of each key, and it blanks the secret boxes again.
        reset(toFormValues(pageSettings(next, page.id)));
      } catch (error) {
        toast.error(error instanceof Error ? error.message : "Could not save settings");
      }
    },
    (errors) => openGroups(Object.keys(errors).map(groupFor)),
  );

  return (
    <div className="space-y-6">
      <PageHeader
        title={page.label}
        subtitle={page.description}
        actions={
          <Button onClick={onSubmit} disabled={!settings || update.isPending}>
            {update.isPending ? "Saving..." : "Save changes"}
          </Button>
        }
      />

      {settingsQuery.isPending ? (
        <div className="space-y-3" data-testid="settings-skeleton">
          <Skeleton className="h-20 w-full" />
          <Skeleton className="h-20 w-full" />
        </div>
      ) : settingsQuery.isError ? (
        <EmptyState
          icon={AlertTriangle}
          title="Could not load settings"
          description={settingsQuery.error.message}
          action={
            <Button variant="outline" onClick={() => void settingsQuery.refetch()}>
              Try again
            </Button>
          }
        />
      ) : groups.length === 0 ? (
        <EmptyState
          icon={AlertTriangle}
          title="Nothing to set here"
          description="The backend reported no settings for this page."
        />
      ) : (
        <form onSubmit={onSubmit} className="space-y-4" noValidate>
          {groups.map(({ group, entries }) => (
            <SectionCard
              key={group.id}
              id={group.id}
              collapsible
              open={isOpen(group.id)}
              onOpenChange={(open) => setGroupOpen(group.id, open)}
              title={group.title}
              description={group.description}
              contentClassName="space-y-5"
            >
              <GroupBody
                page={page.id}
                group={group.id}
                entries={entries}
                control={form.control}
                errors={form.formState.errors}
                disabled={update.isPending}
              />
            </SectionCard>
          ))}
        </form>
      )}
    </div>
  );
}

/**
 * What goes inside one card. Most cards are the generated fields and nothing
 * else; the few that know more than the registry does are named here.
 */
function GroupBody({
  page,
  group,
  entries,
  control,
  errors,
  disabled,
}: {
  page: RegistryPageId;
  group: string;
  entries: ResolvedSetting[];
  control: Control<SettingsFormValues>;
  errors: FieldErrors<SettingsFormValues>;
  disabled: boolean;
}) {
  const props = { entries, control, errors, disabled };
  const fields = entries.map((setting) => (
    <SettingField
      key={setting.key}
      setting={setting}
      control={control}
      error={errors[setting.key]}
      disabled={disabled}
    />
  ));

  switch (`${page}.${group}`) {
    // The password is not a registry field and must not be one: see
    // AuthSection. The username still goes through the generated form.
    case "auth.sign-in":
      return <AuthSection {...props} />;
    // Which LLM keys matter depends on the provider, two are a closed set, and
    // a credential is worth testing before an analysis fails at midnight.
    case "llm.provider":
      return <LlmProviderGroup {...props} />;
    case "llm.models":
      return <LlmModelsGroup {...props} />;
    case "llm.limits":
      return (
        <>
          {fields}
          <RateLimitLatch />
        </>
      );
    case "machine.writes":
      return (
        <>
          <DeviceWritesWarning />
          {fields}
        </>
      );
    default:
      return <>{fields}</>;
  }
}

/**
 * The one setting on this page that can change somebody's espresso machine.
 *
 * Rendered above the field rather than as part of its description, because the
 * description is a sentence in a small grey font under a dropdown and this is
 * the thing a person should read before touching the dropdown. Plain about what
 * goes wrong: a wedged display is recoverable, and the recovery is a reflash
 * plus a filesystem erase.
 */
function DeviceWritesWarning() {
  return (
    <div
      className="rounded-md border border-status-warn/40 bg-status-warn/10 p-3"
      data-testid="device-writes-warning"
    >
      <p className="flex items-center gap-1.5 font-medium text-sm text-status-warn-text">
        <AlertTriangle className="size-3.5" aria-hidden="true" />
        Device writes enabled
      </p>
      <p className="mt-1 text-status-warn-text text-xs">
        Off by default. With it on, gaggiclanker may save a <strong>new</strong> profile to the
        display, delete one it created itself, select one, and star or unstar one. Sending your
        judgements to shots' notes cards and deleting shots the archive already holds happen only
        from the Sync page, when you confirm them. It never overwrites an existing profile and never
        writes device settings. A profile with zero phases crashes brew start on the display and
        recovering that means a reflash plus a filesystem erase — four validation layers stand in
        the way of that, and this switch is the fifth. Every attempt, refused or not, is recorded on
        the{" "}
        <Link className="underline underline-offset-2" to="/sync#writes">
          Sync page
        </Link>
        .
      </p>
    </div>
  );
}
