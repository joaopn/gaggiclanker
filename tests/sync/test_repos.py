...
@pytest.fixture
async def machine_id(db: Database) -> int:
    machine = await MachinesRepository(db).upsert(MachineUpsert(host="machine.local"))
    return machine.id


def a_shot(machine_id: int, device_id: str, *, started_at: str | None = None) -> ShotInsert:
    return ShotInsert(
        device_id=device_id,
...
async def test_a_shot_and_its_samples_land_together(db: Database, machine_id: int) -> None:
    repo = ShotsRepository(db)

    shot_id = await repo.insert(a_shot(machine_id, "000101"), samples())
...
async def test_a_failed_sample_insert_takes_the_shot_with_it(db: Database, machine_id: int) -> None:
    ...
    with pytest.raises(Exception):  # noqa: B017 - the driver's IntegrityError
        await repo.insert(a_shot(machine_id, "000102"), duplicated)

    assert (await repo.counts()).total == 0
    assert await db.fetch_value("SELECT COUNT(*) FROM shot_samples") == 0


async def test_a_quarantined_shot_may_not_carry_samples(db: Database, machine_id: int) -> None:
    repo = ShotsRepository(db)
    shot = a_shot(machine_id, "000103")
...
async def test_deleting_a_shot_takes_its_samples(db: Database, machine_id: int) -> None:
    """ON DELETE CASCADE, which only works because the connection sets foreign_keys."""
    repo = ShotsRepository(db)
    shot_id = await repo.insert(a_shot(machine_id, "000104"), samples())
...
async def test_the_same_shot_cannot_be_stored_twice(db: Database, machine_id: int) -> None:
    """UNIQUE (machine_id, device_id): the diff's idempotence has a backstop."""
    repo = ShotsRepository(db)
    await repo.insert(a_shot(machine_id, "000105"))

    with pytest.raises(Exception):  # noqa: B017 - the driver's IntegrityError
        await repo.insert(a_shot(machine_id, "000105"))


async def test_a_shot_with_no_timestamp_sorts_last(db: Database, machine_id: int) -> None:
    ...
    """
    repo = ShotsRepository(db)
    undated = a_shot(machine_id, "000106")
    undated.started_at = None
    undated.start_epoch = 0
    await repo.insert(undated)
    await repo.insert(a_shot(machine_id, "000107", started_at="2026-01-02T09:00:00.000Z"))
...
async def test_the_cursor_walks_the_whole_archive_exactly_once(
    db: Database, machine_id: int
) -> None:
    repo = ShotsRepository(db)
    for index in range(25):
        await repo.insert(
            a_shot(machine_id, f"{index:06d}", started_at=f"2026-01-01T{index % 24:02d}:00:00.000Z")
        )
...
async def test_counts_split_the_archive_the_way_the_status_page_reads_it(
    db: Database, machine_id: int
) -> None:
    repo = ShotsRepository(db)
    await repo.insert(a_shot(machine_id, "000108"), samples(3))
    quarantined = a_shot(machine_id, "000109")
    quarantined.quarantined = True
    quarantined.quarantine_reason = "bad magic"
    await repo.insert(quarantined)
    deleted = a_shot(machine_id, "000110")
...
async def test_the_machine_upsert_never_blanks_what_it_does_not_know(db: Database) -> None:
    ...
    repo = MachinesRepository(db)
    await repo.upsert(MachineUpsert(host="machine.local", has_pressure=True, name="Gaggia"))

    again = await repo.upsert(MachineUpsert(host="machine.local", display_version="v1.9.0"))
...
async def test_a_shot_can_be_derived_and_stored_without_a_live_device(
    ...
    An importer has a parsed `.slog` and no machine: the shot it is reading was
    deleted from the device months ago, which is the whole reason to import it.
    So the derivation is a free function, and a machine row can be created for a
    synthetic host — the host is the identity and nothing requires it to resolve.
    """
    machine = await MachinesRepository(db).upsert(MachineUpsert(host="import:kitchen-2025"))
